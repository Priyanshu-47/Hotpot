...
function Registration() {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        const response = await axios.get('http://localhost:5249/api/Customer/restaurants');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5249/api/RestaurantOwner', {
        Name: name,
        UserName: username,
        Password: password,
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5249/api/RestaurantOwner/LoginResto', {
        UserName: username,
        Password: password,
        Role: "",
        Token: "",
