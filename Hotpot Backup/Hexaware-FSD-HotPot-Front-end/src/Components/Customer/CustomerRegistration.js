...
function CustomerRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5249/api/CustomerLogin/Register', {
        name: name,
        email: email,
        phone: phone,
        userName: username,
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5249/api/CustomerLogin/LogIn', {
        userName: username,
        password: password,
        role: '',
        token: '',
