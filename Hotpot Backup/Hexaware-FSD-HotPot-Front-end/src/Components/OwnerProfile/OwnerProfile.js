...
const OwnerProfile = () => {
  ...
  useEffect(() => {
    const fetchMenus = async () => {
      try {
        const response = await axios.get('http://localhost:5249/api/Customer/menu');
  ...
  const handleMenuSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5249/api/Customer/menu/add', newMenu);
  ...
  const handleDeleteMenu = async () => {
    try {
      await axios.delete(`http://localhost:5249/api/Customer/menu/delete?id=${selectedMenuId}`);
    ...
    useEffect(() => {
      const fetchOrders = async () => {
        try {
          const response = await axios.get('http://localhost:5249/api/Customer/orders');
  ...
  useEffect(() => {
    const fetchPayments = async  () => {
      try {
        const response = await axios.get('http://localhost:5249/api/Customer/Payments');
  ...
  const handleChangestatus = async (orderId, newStatus) => {
    try {
      await axios.put(`http://localhost:5249/api/Customer/${orderId}/status/${newStatus}`);
